    ...
    <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 relative font-sans xl:h-[calc(100vh-120px)] xl:overflow-hidden">
      <style jsx global>{`
        @media print {
          ...
          #receipt-content { position: absolute !important; left: 0 !important; top: 0 !important; width: 80mm !important; margin: 0 !important; padding: 5mm !important; }
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-8 space-y-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold tracking-wider text-gray-800 uppercase">PEMBAYARAN</h3>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  { id: "QRIS", label: "QRIS", icon: ShoppingBag },
                  { id: "DEBIT", label: "GRAB", icon: Bike },
                ].map((method) => (
                  <button 
                    key={method.id} 
                    type="button" 
                    onClick={() => setPaymentMethod(method.id as any)} 
                    className={`flex flex-col items-center justify-center py-6 rounded-2xl border-2 transition-all ${paymentMethod === method.id ? "bg-orange-50 border-orange-500 text-orange-500 shadow-sm" : "bg-white border-gray-100 text-gray-400 hover:bg-gray-50"}`}
                  >
                    <method.icon className={`h-8 w-8 mb-3 ${paymentMethod === method.id ? "text-orange-500" : "text-gray-300"}`} strokeWidth={1.5} />
              ...
              <div className="space-y-4">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">UANG DITERIMA</label>
              ...
              <div className="bg-orange-50/50 rounded-2xl p-6 border border-orange-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-orange-400 uppercase tracking-wider">Kembalian</span>
                    <span className="text-[10px] text-gray-400">Masukan uang diterima</span>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-lg font-black text-orange-500">
                    {paymentMethod === "CASH" ? `Rp ${changeAmount.toLocaleString()}` : "Rp -"}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                >
                  {isSubmitting ? <Loader2 className="h-5 w-5 animate-spin" /> : <><CheckCircle2 className="h-5 w-5" /><span>Konfirmasi Pembayaran</span></>}
                </button>
                <button 
      ...
      {showReceipt && lastTransaction && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 print:p-0 print:bg-white print:static animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 print:shadow-none print:rounded-none print:max-w-none">
            ...
            <div className="p-8 space-y-4 font-mono text-sm text-gray-800 print:p-0" id="receipt-content">
              <div className="text-center space-y-1">
                ...
                {posConfig.receiptHeader && <p className="text-xs italic pt-2 border-t border-gray-100 mt-2">{posConfig.receiptHeader}</p>}
              </div>
              <div className="border-t border-dashed border-gray-300 pt-4 space-y-1 text-xs">
                <div className="flex justify-between"><span>Order: #{lastTransaction.orderNumber}</span><span>{lastTransaction.createdAt}</span></div>
                <div className="flex justify-between"><span>Metode: {lastTransaction.paymentMethod}</span></div>
              </div>
              <div className="border-t border-dashed border-gray-300 pt-4 space-y-3">
                {lastTransaction.items.map((item, idx) => (
                  <div key={idx} className="space-y-1">
                    <div className="flex justify-between font-bold text-sm"><span>{item.name}</span></div>
                    <div className="flex justify-between text-xs opacity-70"><span>{item.quantity} x {item.price.toLocaleString()}</span><span>{(item.quantity * item.price).toLocaleString()}</span></div>
                  </div>
                ))}
              </div>
              <div className="border-t border-dashed border-gray-300 pt-4 space-y-1 text-xs">
                <div className="flex justify-between"><span>Subtotal</span><span>{lastTransaction.subtotal.toLocaleString()}</span></div>
                {lastTransaction.taxAmount > 0 && <div className="flex justify-between"><span>Pajak ({posConfig.taxPercent}%)</span><span>{lastTransaction.taxAmount.toLocaleString()}</span></div>}
                {lastTransaction.serviceAmount > 0 && <div className="flex justify-between"><span>Service ({posConfig.serviceChargePercent}%)</span><span>{lastTransaction.serviceAmount.toLocaleString()}</span></div>}
                <div className="flex justify-between font-bold text-base pt-3 border-t border-gray-100"><span>TOTAL</span><span>Rp {lastTransaction.totalAmount.toLocaleString()}</span></div>
              </div>
              <div className="border-t border-dashed border-gray-300 pt-4 space-y-1 text-xs">
                <div className="flex justify-between"><span>Bayar</span><span>{lastTransaction.amountReceived.toLocaleString()}</span></div>
                {lastTransaction.paymentMethod === "CASH" && <div className="flex justify-between font-bold text-emerald-600"><span>Kembali</span><span>{lastTransaction.changeAmount.toLocaleString()}</span></div>}
              </div>
            ...
            <div className="p-6 bg-gray-50 border-t flex gap-4 print:hidden">
              <button onClick={handlePrint} className="flex-1 inline-flex items-center justify-center px-4 py-3 bg-gray-900 text-white rounded-xl font-bold text-sm hover:bg-black transition-all transform active:scale-95"><Printer className="h-4 w-4 mr-2" />Struk</button>
              <button onClick={() => setShowReceipt(false)} className="flex-1 px-4 py-3 bg-white border border-gray-200 text-gray-700 rounded-xl font-bold text-sm hover:bg-gray-50 transition-all transform active:scale-95">Selesai</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="xl:col-span-3 space-y-6 xl:overflow-y-auto xl:pr-4 xl:pb-10 scrollbar-thin scrollbar-thumb-gray-200">
        ...
        <div className="relative h-48 md:h-60 w-full overflow-hidden rounded-[2rem] shadow-lg group">
          <img src="https://images.unsplash.com/photo-1513104890138-7c749659a591?q=80&w=1200&auto=format&fit=crop" alt="Promotion Banner" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          <div className="absolute inset-0 bg-linear-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-8">
            <h3 className="text-white text-2xl font-bold mb-1">Paling Laris Minggu Ini!</h3>
            <p className="text-white/90 text-sm">Nikmati diskon spesial untuk kategori Pizza.</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            <button onClick={() => setSelectedCategoryId("all")} className={`shrink-0 relative w-32 h-20 rounded-xl overflow-hidden group transition-all ${selectedCategoryId === "all" ? "ring-2 ring-orange-500" : ""}`}>
              <div className="absolute inset-0 bg-gray-800 transition-opacity group-hover:opacity-80"></div>
              <div className="absolute inset-0 flex items-center justify-center"><span className="text-white font-black text-sm tracking-widest uppercase">ALL</span></div>
            </button>
            {categories.map((cat: any) => {
              const imageUrl = cat.image || categoryImages[cat.name.toUpperCase()] || "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?q=80&w=200&auto=format&fit=crop";
              return (
                <button key={cat.id} onClick={() => setSelectedCategoryId(cat.id)} className={`shrink-0 relative w-32 h-20 rounded-xl overflow-hidden group transition-all ${selectedCategoryId === cat.id ? "ring-2 ring-orange-500" : ""}`}>
                  <img src={imageUrl} className="absolute inset-0 w-full h-full object-cover" alt={cat.name} />
                  <div className="absolute inset-0 bg-black/40 transition-opacity group-hover:opacity-60"></div>
                  <div className="absolute inset-0 flex items-center justify-center"><span className="text-white font-black text-sm tracking-widest uppercase">{cat.name}</span></div>
                </button>
              );
            })}
          </div>
        ...
        <div className="space-y-4">
          ...
          {initialProducts.length === 0 ? (
            <div className="bg-white rounded-3xl p-20 text-center border border-dashed border-gray-200"><p className="text-gray-400 font-medium italic">Belum ada produk tersedia.</p></div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {displayedProducts.map((product) => {
                const availablePortions = productAvailability.get(product.id) || 0;
                ...
                return (
                  <button 
                    ...
                    className={`group relative bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:-translate-y-1 text-left w-full flex flex-col ${isOutOfStock ? "opacity-60 grayscale cursor-not-allowed" : "cursor-pointer active:scale-95"}`}
                  >
                    ...
                    <div className="p-3 pt-1 space-y-1 flex-1 flex flex-col">
                      <h4 className="text-sm font-bold text-gray-800 line-clamp-1 leading-tight group-hover:text-orange-600 transition-colors">{product.name}</h4>
                      <p className="text-[10px] font-bold text-emerald-500">{availablePortions} Tersedia</p>
                      <div className="pt-1"><p className="text-sm font-black text-orange-500">IDR {product.price.toLocaleString()}</p></div>
                    </div>
                  </button>
                );
              })}
          ...
          <div ref={observerTarget} className="h-20 flex items-center justify-center">
            {filteredProducts.length > productLimit ? (
              <div className="flex flex-col items-center gap-2 text-gray-500">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest">Memuat Produk...</span>
              </div>
            ) : filteredProducts.length > 0 ? (
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Semua produk telah ditampilkan</span>
            ) : null}
          </div>
        </div>
      </div>
      ...
      <div className="xl:col-span-1 flex flex-col h-full xl:min-h-0 gap-4 xl:pb-4">
        ...
        <div className="bg-white rounded-[1.5rem] shadow-lg overflow-hidden border border-gray-50 shrink-0">
          <button 
            onClick={() => setIsNewOrderPanelCollapsed(!isNewOrderPanelCollapsed)} 
            className="w-full bg-linear-to-r from-orange-400 to-orange-500 p-4 text-white text-left"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ShoppingCart className="h-4 w-4" />
                <h3 className="text-sm font-bold uppercase tracking-wider">New Order</h3>
              </div>
              <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${isNewOrderPanelCollapsed ? 'rotate-180' : ''}`} />
            </div>
          </button>

          <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isNewOrderPanelCollapsed ? 'max-h-0' : 'max-h-[400px]'}`}>
            <div className="p-4 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">Table</label>
                  <input type="text" placeholder="No Table" value={tableNumber} onChange={(e) => setTableNumber(e.target.value)} className="w-full bg-orange-50 border-0 rounded-lg px-3 py-2 text-xs font-bold text-orange-600 placeholder:text-orange-200 focus:ring-1 focus:ring-orange-100" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1">Name</label>
                  <input type="text" placeholder="Customer" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-orange-50 border-0 rounded-lg px-3 py-2 text-xs font-bold text-orange-600 placeholder:text-orange-200 focus:ring-1 focus:ring-orange-100" />
                </div>
              </div>

              <div>
                <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-1 mb-1.5 block">Order Type</label>
                <div className="grid grid-cols-3 gap-2">
                  {[ { id: "DINE_IN", label: "Dine in", icon: Utensils }, { id: "AWAY", label: "Away", icon: ShoppingBag }, { id: "GRAB", label: "Grab", icon: Bike } ].map((type) => (
                    <button key={type.id} onClick={() => { setOrderType(type.id as "DINE_IN" | "AWAY" | "GRAB"); setIsNewOrderPanelCollapsed(true); }} className={`flex flex-col items-center justify-center py-2 rounded-lg border transition-all ${orderType === type.id ? "bg-orange-50 border-orange-200 text-orange-500 shadow-sm" : "bg-white border-gray-100 text-gray-400 hover:border-orange-100 hover:bg-orange-50/30"}`}>
                      <type.icon className={`h-3.5 w-3.5 mb-1 ${orderType === type.id ? "text-orange-500" : "text-gray-400"}`} />
        ...
        <div className="bg-white rounded-[1.5rem] shadow-lg overflow-hidden flex flex-col flex-1 border border-gray-50 min-h-0 relative">
          ...
          <div className="p-5 pb-3 flex items-center justify-between shrink-0 border-b border-gray-50">
            <div className="flex items-center gap-3">
              <div className="w-[3px] h-6 bg-orange-500 rounded-full"></div>
              <h3 className="text-sm font-bold tracking-wider text-gray-600 uppercase">ITEM PESANAN</h3>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 scrollbar-thin scrollbar-thumb-gray-100 hover:scrollbar-thumb-gray-200">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40">
                ...
                <p className="text-xs font-bold italic text-gray-400 uppercase tracking-widest">Belum ada pesanan</p>
              </div>
            ) : (
              cart.map((item) => (
                <div key={item.product.id} className="flex items-center gap-4 group">
                  <div className="h-14 w-14 rounded-xl overflow-hidden shrink-0 shadow-sm border border-gray-100">
                    <img 
                      src={item.product.image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=100&auto=format&fit=crop"} 
                      className="w-full h-full object-cover" 
                      alt={item.product.name} 
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-gray-800 leading-tight mb-1">{item.product.name}</p>
                    <p className="text-xs font-bold text-orange-500">IDR {item.product.price.toLocaleString()}</p>
                  </div>
          ...
          <div className="p-6 bg-white border-t border-gray-100 space-y-4 shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
            <div className="space-y-2 font-bold">
              ...
              {applyService && (
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-gray-500 uppercase tracking-tighter">Service</span>
                  <span className="text-gray-800">IDR {serviceAmount.toLocaleString()}</span>
                </div>
              )}
              {applyTax && (
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-gray-500 uppercase tracking-tighter">Pajak ({posConfig.taxPercent}%)</span>
                  <span className="text-gray-800">IDR {taxAmount.toLocaleString()}</span>
                </div>
              )}
              <div className="flex items-center justify-between text-sm font-bold text-gray-900 pt-3 mt-1 border-t border-gray-50">
                ...
                <span className="text-lg text-orange-600">IDR {totalAmount.toLocaleString()}</span>
              </div>
            </div>
      ...
      <div className="xl:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] animate-in slide-in-from-bottom duration-300">
        <div className="flex items-center justify-between gap-4 max-w-2xl mx-auto">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Total Tagihan</span>
            <span className="text-lg font-black text-gray-900 leading-none">IDR {totalAmount.toLocaleString()}</span>
          </div>
          <button 
            type="button" 
            onClick={() => {
          ...
          >
            <Banknote className="h-5 w-5" />
            <span>Bayar ({cart.reduce((sum, item) => sum + item.quantity, 0)})</span>
          </button>
        </div>
      </div>
    </div>
